...
ggplot(age_summary, aes(x = age_num, y = entries)) +
  ...
  theme(axis.text.x = element_text(angle = 0, hjust = 1),
        plot.title = element_text(size = 14, face = "bold")) +
  expand_limits(y = max(age_summary$entries) * 1.2)
```
...
income <- income %>%
  ...
  mutate(career_stage_num = case_when(
    career_stage_cat == "Entry Level" ~ 1,
    career_stage_cat == "Growth Phase" ~ 2,
    career_stage_cat == "Peak Earning" ~ 3,
    career_stage_cat == "Retirement Transition" ~ 4))
```
...
career_stage_prop <- income %>%
  group_by(career_stage_cat, income_cat) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(career_stage_cat) %>%
  mutate(prop = count / sum(count))

# Generate plot.

ggplot(career_stage_prop, aes(x = career_stage_cat, y = prop, fill = income_cat)) +
  geom_bar(stat = "identity", position = "fill") +
  scale_fill_manual(values = c("<=50K" = "lightsteelblue", ">50K" = "steelblue")) +
  scale_y_continuous(labels = scales::percent_format()) +
