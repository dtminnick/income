...
```{r}
model_0 <- lm(response_matrix[, "factor(G)0"] ~ ., data = predictors)
model_1 <- lm(response_matrix[, "factor(G)1"] ~ ., data = predictors)

plot(model_0, which = 5)
plot(model_1, which = 5)

```

To assess whether residual structure was due to model misspecification, I extended the linear matrix model with spline terms, higher-order polynomials, and in the last interaction, which I use for this report, interaction terms. However, residual plots continued to show systematic deviations across categorical subgroups and skewed distributions.

This suggests that the residual issues are not due to omitted nonlinear effects, but rather stem from the structural limitations of using a linear regression framework for binary classification. Specifically:

* The residuals are differences between binary outcomes and continuous predictions, which are inherently heteroscedastic and asymmetric, and
* Even with flexible terms, the model cannot fully respect the bounded nature of probabilities or the discrete nature of the response.

To contextualize model selection and assess the tradeoffs in predictive performance and interpretability, I include the linear matrix model in this evaluation. It's performance metrics and residual behavior are analyzed alongside the logistic regression model to provide a comparative baseline and highlight the limitations of linear approaches in binary classification tasks.
