...
income <- income %>%
  mutate(career_stage = case_when(
    ...
    age_num >= 36 & age_num < 60 ~ "Peak Earning",
...
```{r}
model_cs <- glm(income_num ~ career_stage_num, data = income, family = binomial)

income$predicted_prob <- predict(model_cs, type = "response")

```

```{r}
ggplot(income, aes(x = career_stage, y = income_num)) +
  geom_boxplot() +
  labs(title = "Income vs Career Stage", y = "High Income (1 = >50K)", x = "Career Stage")

```

```{r}
ggplot(income, aes(x = career_stage_num, y = predicted_prob)) +
...
```{r define_workclass_group}
income <- income %>%
  mutate(workclass_group_cat = case_when(
    workclass_cat %in% c("Without-pay", "Never-worked") ~ "Not-employed",
...
income <- income %>%
  mutate(education_group = factor(education_group, ordered = TRUE))
```
...
```{r drop_years_of_education}
income <- income %>%
  select(-years_of_education_num)
```
...
```{r define_occupation_group}
income <- income %>%
  mutate(
    occupation_group = case_when(
      occupation_cat %in% c("Prof-specialty", 
...
```{r define_relationship_role}
income <- income %>%
  mutate(relationship_role = case_when(
    relationship_cat %in% c("Husband", "Wife") & hours_per_week_num >= 40 ~ "Primary Earner",
    relationship_cat %in% c("Husband", "Wife") & hours_per_week_num < 40 ~ "Secondary Earner",
...
```{r define_race_income_rate}
race_income_rate <- income %>%
  group_by(race_cat) %>%
  summarise(race_income_score = mean(income_num, na.rm = TRUE))

income <- income %>%
  left_join(race_income_rate, by = "race_cat")
```

### Has Investment Activity

Create a single investment activity flag.

See **Appendices | Appendix A: Exploratory Data Analysis | Capital Gain / Capital Loss** section for rationale.
...
Create a binary flag to captures whether someone is native to the dominant country — useful for income modeling, especially if immigration status or global labor dynamics are relevant.

See **Appendices | Appendix A: Exploratory Data Analysis | Native Country** section for rationale.

```{r define_is_us_native}
income <- income %>%
  mutate(is_us_native = if_else(native_country_cat == "United-States", 1, 0))
```
...
# X <- model.matrix(~ age_num + education_num + marital_status_num - 1, data = train)

X <- model.matrix(~ career_stage +
                    workclass_group_cat +
                    education_level_cat +
                    ...
                    is_us_native - 1, data = train)

# X <- as.matrix(train[, c("career_stage", 
#                          "workclass_group_cat", 
#                          "education_level_cat",
#                          "marital_status_group",
#                          "occupation_group",
...
Metric	Linear	Logistic	Notes
Accuracy	0.8386	0.8379	Nearly identical — both models generalize well
Sensitivity	0.9264	0.9377	Logistic slightly better at detecting >50K earners
Specificity	0.5733	0.5362	Linear better at identifying ≤50K earners
AUC	0.8965	0.8913	Linear edges out logistic — rare but possible when features are well-behaved

That ~22% of linear predictions falling outside the probability range is expected: Linear regression doesn’t constrain outputs, so values <0 or >1 are possible. These can be truncated or thresholded, but they’re not probabilistically valid.

Logistic regression naturally bounds predictions between 0 and 1 via the sigmoid function.

Still, linear model’s AUC and sensitivity suggest it’s capturing the signal remarkably well.

Indicator matrix is well-structured and behaviorally grounded.

Linear regression may be a viable alternative for interpretability, especially in fairness audits or when logistic assumptions don’t hold.

### Coefficient Comparison

```{r}
model_G0 <- fit$coefficients[, "factor(G)0"]
model_G1 <- fit$coefficients[, "factor(G)1"]

# Add names to preserve predictor labels
names(model_G0) <- rownames(fit$coefficients)
names(model_G1) <- rownames(fit$coefficients)
...
To capture non-linear effects of career stage on income classification, we included a second-degree orthogonal polynomial transformation of the numeric career stage variable.  It will model both linear and quadratic trends while avoiding multicollinearity.

We will implement it as `poly(career_stage_num, 2)` when modeling to generate two statistically independent predictors, a linear component (career stage) and a curvature component (career stage squared).
...
# Step 3: Plot
ggplot(income_by_education, aes(x = forcats::fct_reorder(education_cat, education_num), y = prop, fill = income_cat)) +
...
United-States likely accounts for 90%+ of the sample.

Most other countries are sparsely represented — some with only a handful of observations.

Using raw categories risks overfitting to noise or inflating model complexity.
